...
struct Debugger {
    ...
    func checkOnLaunch() -> (Bool, Bool) {
        let wasLaunchedBefore = DefaultsHelper().getBoolean(key: Resources.Keys.keyCheckFirstLaunch)! //Always false if wasn't set, so no nil here
        let isLoggedIn = DefaultsHelper().getBoolean(key: Resources.Keys.keyCheckIfSignedIn)! //Same here
